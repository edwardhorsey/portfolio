const projects = [
...

  {
    ...
    description:
      'A full-stack web app which searches for information on a track release and provides results of DJ mixes which feature the track. The front end (built in React) uses AWS Gateway API and Lambda to conduct the API searches. <span>🔥🎛️</span>',
...

  {
    name: 'Roland-react-8',
    description:
      'A <strong>drum-machine</strong> performing some of my favourite samples from the Roland TR-808. The sample controls are designed for playing live. Distortion included for added techno. Built with React and the Web Audio API. <span>🕺💃</span> ',
